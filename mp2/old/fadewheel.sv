...
module top (
    ...
    // How many states to divide that range into
    parameter DUTY_CYCLE_STATES = 200;
    // How much to increment the duty cycle in each state
    parameter DUTY_CYCLE_INCREMENT = DUTY_CYCLE_RANGE / 200;
    // Create a parameter for the amount of time spent in each PWM duty cycle increment
    parameter PWM_TIME = CYCLE_TIME / DUTY_CYCLE_STATES;
    ...
    always_ff @(posedge clk) begin
        ...
        if(cycle_count == CYCLE_TIME - 1) begin
            ...
            // first.
            if(colorwheel_current_state == NUM_STATES - 2) begin
                colorwheel_current_state <= 0;
            end
            else begin
    ...
    always_ff @(posedge clk) begin
        ...
        if(pwm_duty_count == PWM_TIME - 1) begin
            ...
            if(duty_cycle_inc == DUTY_CYCLE_RANGE - 1) begin
                ...
                duty_cycle_inc <= duty_cycle_inc + DUTY_CYCLE_INCREMENT;
                duty_cycle_dec <= duty_cycle_dec - DUTY_CYCLE_INCREMENT;
            end
        end
    ...
    assign pwm_inc = (pwm_interval_count > duty_cycle_inc) ? 1'b1 : 1'b0;
    assign pwm_dec = (pwm_interval_count > duty_cycle_dec) ? 1'b1 : 1'b0;
